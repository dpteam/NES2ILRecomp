...
namespace NesLifter.Studio
{
    ...
    public class Lifter
    {
        ...
        void AppendApu(StringBuilder sb)
        {
            ...
            sb.AppendLine("    if (addr == 0x4015) Audio.OnStatusChange(value);");
            sb.AppendLine("}");
            sb.AppendLine("}");
            sb.AppendLine();
        }
        ...
        void AppendAudio(StringBuilder sb)
        {
            ...
            sb.AppendLine("    {");
            sb.AppendLine("        byte st = Apu.Regs[0x15];");
            sb.AppendLine("        bool e1 = (st & 1) != 0; bool e2 = (st & 2) != 0; bool eT = (st & 4) != 0; bool eN = (st & 8) != 0;");
            sb.AppendLine("        int per1 = ((Apu.Regs[2] | ((Apu.Regs[3] & 7) << 8)) + 1);");
            sb.AppendLine("        int per2 = ((Apu.Regs[6] | ((Apu.Regs[7] & 7) << 8)) + 1);");
            sb.AppendLine("        int perT = ((Apu.Regs[0xA] | ((Apu.Regs[0xB] & 7) << 8)) + 1);");
            ...
            sb.AppendLine("        int v1 = e1 ? (Apu.Regs[0] & 0xF) : 0;");
            sb.AppendLine("        int v2 = e2 ? (Apu.Regs[4] & 0xF) : 0;");
            sb.AppendLine("        int vT = eT ? 12 : 0;");
            sb.AppendLine("        int vN = eN ? (Apu.Regs[0xC] & 0xF) : 0;");
